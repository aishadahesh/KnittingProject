...
WORKFLOW_STAGES = [
    ("Align", "Match the model and reference image in the viewport."),
    ("Pattern", "Set bitmap size, row visibility, and stitch pattern."),
    ("Geometry", "Tune physical knitting parameters manually."),
    ("Material", "Choose opacity and yarn colors."),
    ("Texture", "Tune the yarn texture tint and see it live."),
    ("Lighting", "Adjust scene lighting and see it live."),
    ("Spline", "Refine control points when the coarse shape is close."),
    ("Review", "Render, save, load, and run the existing optimizer."),
]
...
class MeshRenderer:
    ...
    def render(self, mvp, mv, hover_idx=-1, selected_idx=-1,
               ...
               model_alpha=1.0, texture_tint=(1.0, 1.0, 1.0),
               texture_ridge_strength=0.0, texture_ridge_scale=10.0,
               texture_fiber_strength=0.0, texture_fiber_scale=30.0,
               texture_noise_strength=0.0,
               texture_scale_x=1.0, texture_scale_y=1.0,
               texture_chevron_angle=0.75, texture_fiber_sharpness=1.0,
               texture_groove_darkness=0.0, texture_center_shadow=0.0,
               texture_gloss_strength=0.0, texture_highlight_width=0.35,
               texture_color_band_strength=0.0, texture_color_band_scale=1.0,
               texture_color_band_shift=0.0, texture_saturation=1.0,
        ...
        for vao, n_idx, color in self.meshes:
            ...
            self.prog['texture_tint'].value = tuple(float(c) for c in texture_tint)
            self.prog['texture_ridge_strength'].value = float(texture_ridge_strength)
            self.prog['texture_ridge_scale'].value = float(texture_ridge_scale)
            self.prog['texture_fiber_strength'].value = float(texture_fiber_strength)
            self.prog['texture_fiber_scale'].value = float(texture_fiber_scale)
            self.prog['texture_noise_strength'].value = float(texture_noise_strength)
            self.prog['texture_scale_x'].value = float(texture_scale_x)
            self.prog['texture_scale_y'].value = float(texture_scale_y)
            self.prog['texture_chevron_angle'].value = float(texture_chevron_angle)
            self.prog['texture_fiber_sharpness'].value = float(texture_fiber_sharpness)
            self.prog['texture_groove_darkness'].value = float(texture_groove_darkness)
            self.prog['texture_center_shadow'].value = float(texture_center_shadow)
            self.prog['texture_gloss_strength'].value = float(texture_gloss_strength)
            self.prog['texture_highlight_width'].value = float(texture_highlight_width)
            self.prog['texture_color_band_strength'].value = float(texture_color_band_strength)
            self.prog['texture_color_band_scale'].value = float(texture_color_band_scale)
            self.prog['texture_color_band_shift'].value = float(texture_color_band_shift)
            self.prog['texture_saturation'].value = float(texture_saturation)
            self.prog['texture_contrast'].value = float(texture_contrast)
            self.prog['light_color'].value = tuple(float(c) for c in light_color)
            self.prog['light_intensity'].value = float(light_intensity)
...
def main():
    ...
    s = dict(
        ...
        # Render texture and lighting
        render_texture_color = [0.8, 0.4, 0.3],
        texture_ridge_strength = 0.45,
        texture_ridge_scale = 8.0,
        texture_fiber_strength = 0.35,
        texture_fiber_scale = 34.0,
        texture_noise_strength = 0.18,
        texture_scale_x = 1.0,
        texture_scale_y = 1.0,
        texture_chevron_angle = 0.75,
        texture_fiber_sharpness = 1.8,
        texture_groove_darkness = 0.25,
        texture_center_shadow = 0.18,
        texture_gloss_strength = 0.28,
        texture_highlight_width = 0.28,
        texture_color_band_strength = 0.0,
        texture_color_band_scale = 1.0,
        texture_color_band_shift = 0.0,
        texture_saturation = 1.15,
        texture_contrast = 1.12,
    ...
    texture_param_keys = (
        'texture_ridge_strength', 'texture_ridge_scale',
        'texture_fiber_strength', 'texture_fiber_scale',
        'texture_noise_strength', 'texture_scale_x', 'texture_scale_y',
        'texture_chevron_angle', 'texture_fiber_sharpness',
        'texture_groove_darkness', 'texture_center_shadow',
        'texture_gloss_strength', 'texture_highlight_width',
        'texture_color_band_strength', 'texture_color_band_scale',
        'texture_color_band_shift', 'texture_saturation', 'texture_contrast',
    )
    saved_state_keys = (
        'workflow_step', 'display_copies_x', 'display_copies_y', 'mode',
        'show_ref_bg', 'ref_bg_alpha', 'ref_bg_scale_x', 'ref_bg_scale_y',
        'ref_bg_lock_zoom', 'ref_bg_rotation', 'ref_bg_offset_x',
        'ref_bg_offset_y', 'model_alpha', 'single_model_color',
        'use_row_colors', 'row_colors', 'row_visible', 'render_texture_color',
        *texture_param_keys, 'render_light_color', 'render_light_intensity',
        'mi_cam_dist_mult', 'mi_cam_fov', 'view_fov', 'bitmap_rows',
        'bitmap_cols', 'samples_per_loop', 'model_rot_x', 'model_rot_y',
        'model_rot_z', 'model_tx', 'model_ty', 'model_tz', 'model_drag_mode',
    )
    ...
    def snapshot_state():
        return {
            ...
            'render_texture_color': list(s['render_texture_color']),
            'texture_ridge_strength': s['texture_ridge_strength'],
            'texture_ridge_scale': s['texture_ridge_scale'],
            'texture_fiber_strength': s['texture_fiber_strength'],
            'texture_fiber_scale': s['texture_fiber_scale'],
            'texture_noise_strength': s['texture_noise_strength'],
            'texture_scale_x': s['texture_scale_x'],
            'texture_scale_y': s['texture_scale_y'],
            'texture_chevron_angle': s['texture_chevron_angle'],
            'texture_fiber_sharpness': s['texture_fiber_sharpness'],
            'texture_groove_darkness': s['texture_groove_darkness'],
            'texture_center_shadow': s['texture_center_shadow'],
            'texture_gloss_strength': s['texture_gloss_strength'],
            'texture_highlight_width': s['texture_highlight_width'],
            'texture_color_band_strength': s['texture_color_band_strength'],
            'texture_color_band_scale': s['texture_color_band_scale'],
            'texture_color_band_shift': s['texture_color_band_shift'],
            'texture_saturation': s['texture_saturation'],
            'texture_contrast': s['texture_contrast'],
            'render_light_color': list(s['render_light_color']),
            'render_light_intensity': s['render_light_intensity'],
            'view_fov': s['view_fov'],
    ...
    def restore_snapshot(snap):
        ...
        for key in (
            ...
            'row_colors', 'render_texture_color', 'render_light_color',
            'render_light_intensity', *texture_param_keys, 'view_fov',
    ...
    def do_save_params():
        ...
        for i, name in enumerate(PARAM_NAMES):
            ...
            if span is not None and span > s['bitmap_rows']:
                continue
            params_to_save[name] = float(s['params'][i])
        gui_state = {key: json_ready(s[key]) for key in saved_state_keys if key in s}
        gui_state['camera_dist'] = float(camera.dist)
        gui_state['camera_az'] = float(camera.az)
        gui_state['camera_el'] = float(camera.el)
        gui_state['camera_target'] = json_ready(camera.target)
        data = {
            'params':           params_to_save,
            'bitmap':           s['bitmap'].tolist(),
            'gui_state':        gui_state,
            'mode':             s['mode'],
            'spline_control_rows': [row.tolist() for row in spline.ctrl_rows],
            'single_model_color': s['single_model_color'],
            'use_row_colors':   s['use_row_colors'],
            'row_colors':       s['row_colors'],
            'row_visible':      s['row_visible'].astype(int).tolist(),
            'render_texture_color': s['render_texture_color'],
            'texture_ridge_strength': s['texture_ridge_strength'],
            'texture_ridge_scale': s['texture_ridge_scale'],
            'texture_fiber_strength': s['texture_fiber_strength'],
            'texture_fiber_scale': s['texture_fiber_scale'],
            ...
            'texture_saturation': s['texture_saturation'],
            'texture_contrast': s['texture_contrast'],
            'render_light_color': s['render_light_color'],
            'render_light_intensity': s['render_light_intensity'],
        ...
        try:
            ...
            for key in saved_state_keys:
                if key in gui_state:
                    s[key] = gui_state[key]
            # optional fields
            for key in ('mode', 'single_model_color', 'use_row_colors', 'row_colors',
                        'render_texture_color', 'render_light_color',
                        'render_light_intensity', *texture_param_keys,
                        ...
                        'model_tx', 'model_ty', 'model_tz', 'model_drag_mode'):
                if key in data and key not in gui_state:
                    s[key] = data[key]
            if 'camera_dist' in gui_state:
                camera.dist = float(gui_state['camera_dist'])
            if 'camera_az' in gui_state:
                camera.az = float(gui_state['camera_az'])
            if 'camera_el' in gui_state:
                camera.el = float(gui_state['camera_el'])
            if 'camera_target' in gui_state:
                camera.target = np.array(gui_state['camera_target'], dtype=float)
            if 'row_visible' in gui_state:
                s['row_visible'] = np.array(gui_state['row_visible'], dtype=bool)
            elif 'row_visible' in data:
            ...
            if loaded_spline_rows:
                spline.ctrl_rows = [np.array(row, dtype=np.float32) for row in loaded_spline_rows]
                spline._rebuild()
                if s['mode'] == 'spline':
                    rebuild_spline_mesh()
        except Exception as e:
    ...
    def bg_render():
        try:
            ...
            scene = optimizer.get_scene_dict(path + "_combined.obj", s['params'], camera_params=cam_params)
            scene['mesh']['bsdf']['reflectance']['value'] = [
                float(v) for v in s['render_texture_color']
            ]
            scene['emitter']['radiance']['value'] = [
                float(s['render_light_intensity']) * float(v)
                for v in s['render_light_color']
            ]
    ...
    def current_stage():
        return int(np.clip(s['workflow_step'], 0, len(WORKFLOW_STAGES) - 1))

    def is_stage(index):
        return current_stage() == index

    def draw_workflow_header():
        ...
        if imgui.button("Back##workflow", (nav_w, 0)):
            s['workflow_step'] = max(0, stage_idx - 1)
        if back_disabled:
            imgui.end_disabled()
        imgui.same_line()
        if next_disabled:
        ...
        if imgui.button("Next##workflow", (nav_w, 0)):
            s['workflow_step'] = min(len(WORKFLOW_STAGES) - 1, stage_idx + 1)
        if next_disabled:
            imgui.end_disabled()
        imgui.separator()
    ...
    while not glfw.window_should_close(window):
        ...
        if stage == 0:
            ...
            alignment_locked = bool(s['show_ref_bg'] and s['ref_bg_lock_zoom'])
            if s['show_ref_bg']:
                old_lock_zoom = s['ref_bg_lock_zoom']
                changed_lock, new_lock_zoom = imgui.checkbox("Lock image/model alignment##bg", s['ref_bg_lock_zoom'])
                if changed_lock:
                    zoom_factor = max(float(camera.zoom_factor()), 1e-6)
                    old_zoom_scale = zoom_factor if old_lock_zoom else 1.0
                    new_zoom_scale = zoom_factor if new_lock_zoom else 1.0
                    visible_scale_x = s['ref_bg_scale_x'] * old_zoom_scale
                    visible_scale_y = s['ref_bg_scale_y'] * old_zoom_scale
                    s['ref_bg_lock_zoom'] = old_lock_zoom
                    push_undo("Background lock")
                    s['ref_bg_lock_zoom'] = new_lock_zoom
                    s['ref_bg_scale_x'] = visible_scale_x / new_zoom_scale
                    s['ref_bg_scale_y'] = visible_scale_y / new_zoom_scale
                _, s['ref_bg_alpha'] = imgui.slider_float("Opacity##bg", s['ref_bg_alpha'], 0.0, 1.0, "%.2f")
                alignment_locked = bool(s['ref_bg_lock_zoom'])
                if alignment_locked:
                    imgui.begin_disabled()
                _, s['ref_bg_scale_x'] = imgui.drag_float("Image width##bgsx", s['ref_bg_scale_x'], 0.01, 0.01, 50.0, "%.2f")
                _, s['ref_bg_scale_y'] = imgui.drag_float("Image height##bgsy", s['ref_bg_scale_y'], 0.01, 0.01, 50.0, "%.2f")
                if alignment_locked:
                    imgui.end_disabled()
                    imgui.text_disabled("Alignment locked. Unlock to move image or model.")
                if s['ref_bg_lock_zoom']:
                    zoom_factor = camera.zoom_factor()
                    imgui.text_disabled(
                        f"Displayed: {s['ref_bg_scale_x'] * zoom_factor:.2f} x "
                        f"{s['ref_bg_scale_y'] * zoom_factor:.2f}"
                    )
                if alignment_locked:
                    imgui.begin_disabled()
                ...
                _, s['ref_bg_offset_x'] = imgui.drag_float("Image X##bgox", s['ref_bg_offset_x'], 0.001, -2.0, 2.0, "%.3f")
                _, s['ref_bg_offset_y'] = imgui.drag_float("Image Y##bgoy", s['ref_bg_offset_y'], 0.001, -2.0, 2.0, "%.3f")
                if imgui.small_button("Center image##bg"):
                    s['ref_bg_offset_x'] = s['ref_bg_offset_y'] = 0.0
                _, s['ref_bg_rotation'] = imgui.slider_float("Image rotation##bg", s['ref_bg_rotation'], -float(np.pi), float(np.pi), "%.2f rad")
                if alignment_locked:
                    imgui.end_disabled()

            imgui.separator()
            changed_view_fov, new_view_fov = imgui.slider_float("View FoV##view", s['view_fov'], 10.0, 120.0, "%.1f")
            ...
            for key, label in (("model_rot_x", "X"), ("model_rot_y", "Y"), ("model_rot_z", "Z")):
                changed, val = imgui.slider_float(f"{label}##align_{key}", s[key], -pi, pi, f"{label}: %.2f rad")
                if imgui.is_item_activated():
                    push_undo("Model rotation")
                if changed:
                    s[key] = val
            imgui.text_colored((0.4, 0.8, 0.8, 1.0), "Position")
            for key, label in (("model_tx", "X"), ("model_ty", "Y"), ("model_tz", "Z")):
                changed, val = imgui.drag_float(f"{label}##align_{key}", s[key], 0.01, -100.0, 100.0, f"{label}: %.3f")
                if imgui.is_item_activated():
                    push_undo("Model position")
                if changed:
                    s[key] = val
            if imgui.small_button("Center model##align"):
                push_undo("Center model")
            ...
            max_rows = len(LOOP_HEIGHT_PARAM_INDICES)
            ch_r, new_rows = imgui.slider_int("Rows##bres", s['bitmap_rows'], 1, max_rows)
            ch_c, new_cols = imgui.slider_int("Columns##bres", s['bitmap_cols'], 1, 16)
            if ch_r or ch_c:
            ...
            for r in range(s['bitmap_rows']):
                changed_row, new_visible = imgui.checkbox(f"Row {r + 1}##row_vis_{r}", bool(s['row_visible'][r]))
                if changed_row:
                    if not row_changed:
            ...
            for i, name in enumerate(PARAM_NAMES):
                ...
                if span is not None and span > s['bitmap_rows']:
                    continue
                lo, hi = PARAM_RANGES[i]
                changed, new_val = imgui.slider_float(f"##p{i}", s['params'][i], lo, hi, format=f"{name}: %.3f")
                if imgui.is_item_activated():
                    push_undo(name)
                if changed:
                    s['params'][i] = new_val
            ...
            imgui.text("Material")
            _, s['model_alpha'] = imgui.slider_float("Opacity##mdl", s['model_alpha'], 0.0, 1.0, "%.2f")
            ...
            if not s['use_row_colors']:
                changed_c, new_col = imgui.color_edit3("One color for all##single_color", (float(s['single_model_color'][0]), float(s['single_model_color'][1]), float(s['single_model_color'][2])))
                if imgui.is_item_activated():
            ...
            changed_tex, new_tex = imgui.color_edit3(
                "Tint##render_texture",
                (
                    float(s['render_texture_color'][0]),
                    float(s['render_texture_color'][1]),
                    float(s['render_texture_color'][2]),
                ),
            )
            if imgui.is_item_activated():
                push_undo("Render texture")
            if changed_tex:
                s['render_texture_color'] = list(new_tex)
            if imgui.small_button("Neutral tint##texture"):
                push_undo("Render texture")
                s['render_texture_color'] = [1.0, 1.0, 1.0]
            imgui.same_line()
            if imgui.small_button("Copy material##texture"):
                push_undo("Render texture")
                s['render_texture_color'] = list(s['single_model_color'])
            imgui.separator()
            imgui.text("Scale and direction")
            for key, label, lo, hi, fmt in (
                ('texture_scale_x', 'Scale X', 0.25, 4.0, '%.2f'),
                ('texture_scale_y', 'Scale Y', 0.25, 4.0, '%.2f'),
                ('texture_chevron_angle', 'Chevron angle', -1.57, 1.57, '%.2f rad'),
            ):
                changed, new_val = imgui.slider_float(f"{label}##{key}", s[key], lo, hi, fmt)
                if imgui.is_item_activated():
                    push_undo(label)
                if changed:
                    s[key] = new_val
            imgui.separator()
            imgui.text("Fibers and roughness")
            for key, label, lo, hi, fmt in (
                ('texture_ridge_strength', 'Stitch ridge depth', 0.0, 1.0, '%.2f'),
                ('texture_ridge_scale', 'Stitch ridge spacing', 1.0, 24.0, '%.1f'),
                ('texture_fiber_strength', 'Fiber streak depth', 0.0, 1.0, '%.2f'),
                ('texture_fiber_scale', 'Fiber streak spacing', 4.0, 80.0, '%.1f'),
                ('texture_fiber_sharpness', 'Fiber sharpness', 0.35, 4.0, '%.2f'),
                ('texture_noise_strength', 'Uneven roughness', 0.0, 1.0, '%.2f'),
            ):
                changed, new_val = imgui.slider_float(f"{label}##{key}", s[key], lo, hi, fmt)
                if imgui.is_item_activated():
                    push_undo(label)
                if changed:
                    s[key] = new_val
            imgui.separator()
            imgui.text("Stitch shadows")
            for key, label, lo, hi, fmt in (
                ('texture_groove_darkness', 'Dark groove depth', 0.0, 1.0, '%.2f'),
                ('texture_center_shadow', 'Center crease shadow', 0.0, 1.0, '%.2f'),
            ):
                changed, new_val = imgui.slider_float(f"{label}##{key}", s[key], lo, hi, fmt)
                if imgui.is_item_activated():
                    push_undo(label)
            ...
            imgui.text("Gloss")
            for key, label, lo, hi, fmt in (
                ('texture_gloss_strength', 'Gloss strength', 0.0, 2.0, '%.2f'),
                ('texture_highlight_width', 'Highlight width', 0.0, 1.0, '%.2f'),
            ):
                changed, new_val = imgui.slider_float(f"{label}##{key}", s[key], lo, hi, fmt)
                if imgui.is_item_activated():
                    push_undo(label)
                if changed:
                    s[key] = new_val
            imgui.separator()
            imgui.text("Reference color pattern")
            for key, label, lo, hi, fmt in (
                ('texture_color_band_strength', 'Blue/yellow/red/brown mix', 0.0, 1.0, '%.2f'),
                ('texture_color_band_scale', 'Color repeat scale', 0.1, 8.0, '%.2f'),
                ('texture_color_band_shift', 'Color phase', -12.0, 12.0, '%.2f'),
                ('texture_saturation', 'Saturation', 0.0, 2.5, '%.2f'),
                ('texture_contrast', 'Contrast', 0.25, 2.5, '%.2f'),
            ):
                changed, new_val = imgui.slider_float(f"{label}##{key}", s[key], lo, hi, fmt)
                if imgui.is_item_activated():
                    push_undo(label)
                if changed:
                    s[key] = new_val
            imgui.separator()
            if imgui.small_button("Soft yarn##texture"):
                push_undo("Texture preset")
                s['texture_ridge_strength'] = 0.35
                s['texture_ridge_scale'] = 7.0
                s['texture_fiber_strength'] = 0.25
                s['texture_fiber_scale'] = 28.0
                s['texture_noise_strength'] = 0.12
                s['texture_fiber_sharpness'] = 1.2
                s['texture_groove_darkness'] = 0.14
                s['texture_center_shadow'] = 0.08
                s['texture_gloss_strength'] = 0.14
                s['texture_highlight_width'] = 0.38
            imgui.same_line()
            if imgui.small_button("Strong fibers##texture"):
                push_undo("Texture preset")
                s['texture_ridge_strength'] = 0.65
                s['texture_ridge_scale'] = 11.0
                s['texture_fiber_strength'] = 0.75
                s['texture_fiber_scale'] = 52.0
                s['texture_noise_strength'] = 0.28
                s['texture_fiber_sharpness'] = 2.8
                s['texture_groove_darkness'] = 0.36
                s['texture_center_shadow'] = 0.22
                s['texture_gloss_strength'] = 0.45
                s['texture_highlight_width'] = 0.20
            if imgui.small_button("Reference-like##texture"):
                push_undo("Texture preset")
                s['texture_scale_x'] = 1.25
                s['texture_scale_y'] = 0.85
                s['texture_chevron_angle'] = 0.82
                s['texture_ridge_strength'] = 0.72
                s['texture_ridge_scale'] = 9.5
                s['texture_fiber_strength'] = 0.72
                s['texture_fiber_scale'] = 58.0
                s['texture_fiber_sharpness'] = 3.0
                s['texture_noise_strength'] = 0.22
                s['texture_groove_darkness'] = 0.48
                s['texture_center_shadow'] = 0.34
                s['texture_gloss_strength'] = 0.65
                s['texture_highlight_width'] = 0.16
                s['texture_color_band_strength'] = 0.75
                s['texture_color_band_scale'] = 1.15
                s['texture_color_band_shift'] = 0.0
                s['texture_saturation'] = 1.35
                s['texture_contrast'] = 1.35
            if imgui.small_button("Clear texture##texture"):
                push_undo("Texture reset")
                s['render_texture_color'] = [1.0, 1.0, 1.0]
                s['texture_ridge_strength'] = 0.0
                s['texture_ridge_scale'] = 8.0
                s['texture_fiber_strength'] = 0.0
                s['texture_fiber_scale'] = 34.0
                s['texture_noise_strength'] = 0.0
                s['texture_scale_x'] = 1.0
                s['texture_scale_y'] = 1.0
                s['texture_chevron_angle'] = 0.75
                s['texture_fiber_sharpness'] = 1.8
                s['texture_groove_darkness'] = 0.0
                s['texture_center_shadow'] = 0.0
                s['texture_gloss_strength'] = 0.0
                s['texture_highlight_width'] = 0.28
                s['texture_color_band_strength'] = 0.0
                s['texture_color_band_scale'] = 1.0
                s['texture_color_band_shift'] = 0.0
                s['texture_saturation'] = 1.0
                s['texture_contrast'] = 1.0
            ...
            changed_light, new_light = imgui.color_edit3(
                ...
                (
                    float(s['render_light_color'][0]),
                    float(s['render_light_color'][1]),
                    float(s['render_light_color'][2]),
                ),
            )
            if imgui.is_item_activated():
                push_undo("Light color")
            if changed_light:
                s['render_light_color'] = list(new_light)
            changed_intensity, new_intensity = imgui.slider_float(
                "Light intensity##render_light",
                s['render_light_intensity'],
                0.05,
                3.0,
                "%.2f",
            )
            if imgui.is_item_activated():
                push_undo("Light intensity")
            if changed_intensity:
                s['render_light_intensity'] = new_intensity
            if imgui.small_button("Reset lighting##render_light"):
                push_undo("Lighting")
                s['render_light_color'] = [1.0, 1.0, 1.0]
                s['render_light_intensity'] = 0.9
            ...
            if imgui.radio_button("Spline", s['mode'] == 'spline') and s['mode'] != 'spline':
                ...
                spline.init_from_params(s['params'])
                rebuild_spline_mesh()
            ch_spl, new_spl = imgui.slider_int("Samples/loop##spl", s['samples_per_loop'], 2, 20)
            if ch_spl:
            ...
            if changed_x or changed_y:
                ...
                rebuild_current_mesh()
            _, s['mi_cam_dist_mult'] = imgui.slider_float("Render dist mult##mi", s['mi_cam_dist_mult'], 0.3, 3.0, "%.2f")
            _, s['mi_cam_fov'] = imgui.slider_float("Render FoV##mi", s['mi_cam_fov'], 10.0, 120.0, "%.1f")
            imgui.separator()
        ...
        renderer.render(
            ...
            model_alpha = s['model_alpha'],
            texture_tint = s['render_texture_color'],
            texture_ridge_strength = s['texture_ridge_strength'],
            texture_ridge_scale = s['texture_ridge_scale'],
            texture_fiber_strength = s['texture_fiber_strength'],
            texture_fiber_scale = s['texture_fiber_scale'],
            ...
            texture_saturation = s['texture_saturation'],
            texture_contrast = s['texture_contrast'],
            light_color = s['render_light_color'],
            light_intensity = s['render_light_intensity'],
        )
        ...
        if is_hovered:
            ...
            if prev_mouse:
                ...
                if s['mode'] == 'spline' and s['selected_idx'] >= 0 and (
                    ...
                    lmb_rotates_model = not alignment_locked
            ...
            if (not alignment_locked
                    and glfw.get_mouse_button(window, glfw.MOUSE_BUTTON_LEFT) == glfw.PRESS):
