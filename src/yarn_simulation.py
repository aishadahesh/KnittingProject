...
# Per-step solver logging. The GUI runs run_simulation_step continuously on a
# background thread (~6 steps/s), and each step emits several lines, so this is
# off by default; set yarn_simulation.SIM_DEBUG = True to trace a solve.
SIM_DEBUG = False
...
def run_simulation_step(ctrl_rows, period_offset_x, period_offset_y, config, J_cached, L0_array, k_s, k_b, k_c, dhat):
    ...
    if not success:
        if debug:
            print("[Sim Step] FAILED: line search could not find energy decrease.")
